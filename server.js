var http = require('http'),
    url = require('url'),
    fs = require('fs');
var server1 = http.createServer(function (request, response) {//回调函数--request, response
    //console.log(typeof request);// object 对象格式
    //客户端请求一次就触发一次callback，
    //request：存储了客户端传递进来的所有信息。？？？问号传参过来的还是请求主体中的？？？都有
    //response：提供了很多的方法，可以让服务器端把内容返回给客户端 response.writeHead  response.end
    var urlObj = url.parse(request.url, true),
    //默认false，query是字符串格式--> query: 'name=zhuzhu&age=26',
    //设置为true，query就变成了对象格式--> query: { name: 'zhuzhu', age: '26' }
...
server1.listen(84, function () {
    console.log('server is success,listing on 84 port!');
});